...
*/
  ...
  bubble_x = mapf(accel_event.acceleration.x, 
                                      X_LEVEL_LEFT, X_LEVEL_RIGHT, 
                                      screen_mid_x - largecircle_r + BUBBLE_R + 2,
                                      screen_mid_x + largecircle_r - BUBBLE_R - 2);
  bubble_y = mapf(accel_event.acceleration.y,
  ...
  if ((abs(bubble_x - screen_mid_x) < (largecircle_r / 4 - BUBBLE_R)) &&
    ...
    canvas->fillCircle(bubble_x, bubble_y, BUBBLE_R, __builtin_bswap16(ARCADA_GREEN));
  } else {
    canvas->fillCircle(bubble_x, bubble_y, BUBBLE_R, __builtin_bswap16(ARCADA_RED));    
  }
}
